...
import CategoryWeddingHero from '../../assets/images/product-listing-images/category-wedding-hero.mp4'
...
const AllProducts = ({ allProducts,
  ...
  useEffect(() => {
    document.title = `${activeCategory.charAt(0).toUpperCase() + activeCategory.slice(1)} | Lady Luxe`;
  }, [activeCategory]);
  ...
  return (
    <article>
      <article className="products-hero">
        <h2>{activeCategory === 'all' ? "Boost your style sense!" : activeCategory === 'Western Wear' ? "Let's create your own style" : activeCategory === 'Crop Top' ? "The joy of dressing" : activeCategory === 'shoes' ? "Unlock your style" : activeCategory === 'jeans' ? "Fashion never sleeps" : activeCategory === 'Wedding suits' ? "Embrace elegance on your special day" : "Boost your style sense!"}</h2>
        <video autoPlay loop muted playsInline key={activeCategory}>
          <source src={activeCategory === 'all' ? CategoryAllHero : activeCategory === 'Western Wear' ? CategoryDressesHero : activeCategory === 'Wedding suits' ? CategoryWeddingHero : activeCategory === 'Crop Top' ? CategoryBlousesHero : activeCategory === 'shoes' ? CategoryShoesHero : activeCategory === 'jeans' ? CategoryJeansHero : CategoryAllHero} type="video/mp4" />
        </video>
      </article>
